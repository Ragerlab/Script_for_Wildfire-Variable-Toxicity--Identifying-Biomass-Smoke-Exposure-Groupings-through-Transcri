...
# ggsave("figures/4h_optimal_clusters_samp_wss.png", height=10, width=16, unit="in",plot=wssplot_samp)
# 
...
# ggsave("figures/4h_optimal_clusters_samp_silhouette.png", height=10, width=16, unit="in",plot=silhoplot_samp)
...
# ggsave("figures/4h_optimal_clusters_gene_wss.png", height=10, width=16, unit="in",plot=wssplot_gene)
# Sys.time()
...
# ggsave("figures/4h_optimal_clusters_gene_silhouette.png", height=10, width=16, unit="in",plot=silhoplot_gene)
# Sys.time()
...
#Select 40 clusters and get cluster assignment for each gene
ncut_gene <- 40
...
#Make heatmap
pheatmap(hm_data, main="4h (padj<0.1 & |FC|>=2.5)",
         cluster_rows=FALSE,
         cluster_cols = FALSE,
         show_rownames = TRUE,
         show_colnames = FALSE,
         ...
         filename = "figures/4h_omics_heatmap.png",
         height = 24,
         width = 36,
         border_color = NA)
...
cp <- cp %>% select(rownames(keep)) %>% rownames_to_column("ID") %>% separate(ID, into = c("cond", "time"),sep = '_') %>% filter(time==4) %>% select(!time) %>% arrange(factor(cond, levels=rownames(hm_data))) %>% column_to_rownames("cond")

# make heatmap
pheatmap(cp, main="4h Cardiopulm (p<0.1 & |FC|>=2.5)",
         cluster_rows=FALSE,
         cluster_cols = FALSE,
         show_rownames = TRUE,
         show_colnames = TRUE,
         ...
         angle_col = 45,
         gaps_row = seprows,
         filename = "figures/4h_tox_heatmap.png",
         height = 24,
         width = 36)
...
final_tab <- final_tab %>% select(!merge_col) %>%
  ...
  select(c(1,4,15,5,16,2,13,3,14,8,19,,11,22,7,18,9,20,10,21,6,17,12,23)) %>% 
  slice(match(rownames(sim_mat),ind))


write_csv(final_tab, "output/4h_similarity_lower_tri.csv")
...
for(i in clusters){
  ...
  write.table(temp, file=paste0("output/4h_IPA_input_hm_cluster_",ct,".txt"), sep="\t",row.names = FALSE)
  ct <- ct+1
}
