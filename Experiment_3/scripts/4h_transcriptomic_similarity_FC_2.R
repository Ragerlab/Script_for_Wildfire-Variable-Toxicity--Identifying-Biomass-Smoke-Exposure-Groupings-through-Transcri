...
sig <- full_df %>% filter(expo_cond!="LPS") %>%  filter(padj<0.1 & abs(log2FoldChange)>=log2(2))
sig <- unique(sig$gene)
...
df_sig <- full_df %>%
  ...
  mutate(expr=if_else(padj<0.1 & abs(log2FoldChange)>=log2(2) ,1,0)) %>%
...
ord <- read_csv("input/4h_similarity_lower_tri.csv")
seprows <- c(2,6,7,9)
...
# ggsave("figures/4h_optimal_clusters_gene_wss_FC_2.png", height=10, width=16, unit="in",plot=wssplot_gene)
# Sys.time()
...
# ggsave("figures/4h_optimal_clusters_gene_silhouette_FC_2.png", height=10, width=16, unit="in",plot=silhoplot_gene)
# Sys.time()
...
#Select 69 clusters and get cluster assignment for each gene
ncut_gene <- 69
...
#Make heatmap
pheatmap(hm_data, main="4h (padj<0.1 & |FC|>=2)",
         cluster_rows=FALSE,
         cluster_cols = FALSE,
         show_rownames = TRUE,
         show_colnames = FALSE,
         ...
         filename = "figures/4h_omics_heatmap_FC_2.png",
         height = 24,
         width = 36,
         border_color = NA)
...
write_csv(final_tab, "output/4h_similarity_lower_tri_FC_2.csv")
